...
function navigate(page) {
  ...
  if (window.location.hash.slice(1) === page) renderPage();
}

function initRouting() {
  window.addEventListener("hashchange", () => {
    _currentPage = window.location.hash.slice(1) || "dashboard";
    renderPage();
  });
  _currentPage = window.location.hash.slice(1) || "dashboard";
}
...
function renderNav() {
  ...
  nav.innerHTML = NAV.map(group =>
    ...
    group.items.map(item =>
      `<a
        class="nav-item${_currentPage === item.id ? " active" : ""}"
        href="${item.href || "#" + item.id}"
        data-nav="${item.id}"
        data-external="${item.href ? "true" : "false"}"
        aria-current="${_currentPage === item.id ? "page" : "false"}"
      >
