...
type Tab = "Pipeline" | "Leads" | "Documents" | "Deliveries" | "Call Center";
...
export default function App(){
  ...
  useEffect(()=>{let alive=true;const load=()=>fetch("/api/buddy-events?limit=2000").then(r=>r.ok?r.json():Promise.reject()).then(p=>{if(p.ok!==true)throw new Error();const data=p.data;if(alive){setConversations(data.conversations||[]);setEvents(data.events||[]);setEventsLive(true);setSelectedCall(current=>current?((data.conversations||[]).find((c:Conversation)=>c.callSid===current.callSid)||null):((data.conversations||[])[0]||null));}}).catch(()=>{if(alive)setEventsLive(false);});void load();const timer=window.setInterval(load,8000);return()=>{alive=false;clearInterval(timer);};},[revision]);
  ...
  async function act(lead:Lead,action:LeadAction){
    ...
    try{
      ...
      if(action==="call"){
        ...
        setView("Operations");setTab("Call Center");
        return;
      }
      if(action==="sms"){
        if(!lead.phone)throw new Error("This customer has no phone number.");
  ...
  return <div className="buddy-app">
    ...
    <main className="workspace"><header className="topbar"><div><span className="eyebrow">BUDDY'S HOME FURNISHINGS</span><h1>Personal Shopper Operations</h1><p>Lead to conversation to signed agreement to delivery.</p></div><div className="top-actions"><div className="search"><Search size={17}/><span>Search customers...</span></div><div className="avatar">JS</div></div></header>
      ...
      {view==="Operations"&&<><div className="tabbar">{(["Pipeline","Leads","Documents","Deliveries","Call Center"] as Tab[]).map(t=><button key={t} onClick={()=>setTab(t)} className={tab===t?"tab active":"tab"}>{t}</button>)}</div>{tab==="Call Center"?<CallCenter leads={leads} live={live} selected={selected} onSelect={setSelected} conversations={conversations} eventsLive={eventsLive} refresh={()=>setRevision(r=>r+1)}/>:<div className="content-shell"><section className="content-main">{tab==="Pipeline"&&<Pipeline leads={leads} onSelect={setSelected} onAction={act}/>} {tab==="Leads"&&<Leads leads={leads} onSelect={setSelected} onAction={act}/>} {tab==="Documents"&&<Documents leads={leads} onSelect={setSelected}/>} {tab==="Deliveries"&&<Deliveries leads={leads} onSelect={setSelected}/>}</section><LeadDetail lead={selected} onAction={act}/></div>}</>}
    ...
    </main><SupportGuide navigate={next=>{setView("Operations");setTab(next as Tab);}}/>
  </div>;
}
