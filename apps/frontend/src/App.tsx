...
import {
  BarChart3, CalendarDays, CheckCircle2, ExternalLink, UserRoundPlus, MessagesSquare, BadgeCheck, Truck, TrendingUp,
  FileSignature, FileText, LayoutDashboard, Mail, MessageSquare,
  Phone, ShoppingCart, Users
} from "lucide-react";
import "./App.css";
import "./CrmPremium.css";
...
export type Lead = {
  optedOut?:boolean; comments?:string; id:string; firstName:string; lastName:string; phone?:string; email?:string;
...
export type Conversation = { callSid?:string; contactId?:string; startedAt:number; endedAt:number; transcript:{role:string;text:string;at:number;type?:string}[]; events:BuddyEvent[] };
type Tab = "Pipeline" | "Leads" | "Documents" | "Deliveries";
type View = "Operations" | "Customers" | "Call Center" | "Analytics";
...
function normalizeContact(raw:any,i:number):Lead {
  ...
  return {
    optedOut:Boolean(d?.optedOut),comments:d?.comments||"",id:String(d?.id||raw?.id||`lead-${i}`),firstName:d?.firstName||d?.first_name||d?.name?.split?.(" ")?.[0]||"Guest",
    ...
    location:d?.location||d?.state||d?.area||"Unassigned",stage:inferStage(d),score:(d?.leadScore??d?.score)!=null&&Number.isFinite(Number(d?.leadScore??d?.score))?Number(d?.leadScore??d?.score):undefined,source:d?.source||"Buddy web lead",
...
export default function App(){
  const [view,setView]=useState<View>("Operations"),[tab,setTab]=useState<Tab>("Pipeline"),[leads,setLeads]=useState<Lead[]>([]),[selected,setSelected]=useState<Lead|null>(null),[live,setLive]=useState(false);
  const [conversations,setConversations]=useState<Conversation[]>([]),[events,setEvents]=useState<BuddyEvent[]>([]);
  const [revision,setRevision]=useState(0),[eventsLive,setEventsLive]=useState(false),[crmDataError,setCrmDataError]=useState("");

  useEffect(()=>{let alive=true;const load=async()=>{try{const r=await fetch("/api/contacts",{headers:{Accept:"application/json"}});const payload=await r.json().catch(()=>({}));if(!r.ok||payload?.ok===false)throw new Error(payload?.error||`Lead data failed (${r.status})`);const rows=Array.isArray(payload)?payload:payload?.contacts||payload?.rows||payload?.data;if(!Array.isArray(rows))throw new Error("Lead data returned an invalid response.");if(alive){const mapped=rows.map(normalizeContact);setLeads(mapped);setSelected(current=>mapped.find((l:Lead)=>l.id===current?.id)||mapped[0]||null);setLive(true);setCrmDataError("");}}catch(error:any){if(alive){setLive(false);setCrmDataError(error?.message||"Lead data unavailable.");}}};void load();const timer=window.setInterval(load,5000);return()=>{alive=false;clearInterval(timer);};},[revision]);
  useEffect(()=>{let alive=true;const load=()=>fetch("/api/buddy-events?limit=2000").then(r=>r.ok?r.json():Promise.reject()).then(p=>{if(p.ok!==true)throw new Error();const data=p.data;if(alive){setConversations(data.conversations||[]);setEvents(data.events||[]);setEventsLive(true);}}).catch(()=>{if(alive)setEventsLive(false);});void load();const timer=window.setInterval(load,8000);return()=>{alive=false;clearInterval(timer);};},[revision]);
  ...
  const accessLocked=/forbidden|contacts:read|operator access/i.test(crmDataError);
  const metricValue=(value:any)=>crmDataError?"—":value;
  ...
  return <div className="buddy-app">
    <aside className="side-nav"><div className="brand"><div className="brand-mark">B</div><div><strong>BUDDY'S</strong><span>Sales Center</span></div></div><nav>
      ...
      <button onClick={()=>setView("Call Center")} className={view==="Call Center"?"nav-item active":"nav-item"}><Phone size={19}/> Call Center</button>
      <button onClick={()=>setView("Analytics")} className={view==="Analytics"?"nav-item active":"nav-item"}><BarChart3 size={19}/> Analytics</button>
    </nav><div className="side-status"><span className={live?"dot live":"dot"}/>{live?"Live data":crmDataError?"Operator data locked":"Connecting..."}<small>AI sales workflow</small></div></aside>

    <main className="workspace"><header className="topbar crm-banner"><div className="crm-buddy" role="img" aria-label="Buddy standing in his showroom"/><div className="crm-brand-copy"><img src="/buddys/images/BHF_Logo.webp" alt="Buddy's Home Furnishings"/><div className="crm-title-copy"><h1>Personal Shopper Operations</h1><p>Your home. Your style. Your Buddy.</p></div></div><div className="crm-banner-actions"><a href="/buddys/#contact-form">Open sales experience <ExternalLink size={14}/></a><span className="crm-powered">Powered by <b>BLACK HOLE AI</b> · EILA OS</span></div></header>
      {crmDataError&&<section className="crm-data-alert" role="alert"><div><b>{accessLocked?"Operator access required":"CRM data unavailable"}</b><span>{accessLocked?"The public lead form can still receive requests, but this dashboard is not authorized to read /api/contacts. Configure or sign in to Buddy operator access, then retry.":crmDataError}</span></div><button type="button" onClick={()=>setRevision(r=>r+1)}>Retry data</button></section>}
      <section className="kpis"><Kpi label="Total Leads" value={metricValue(metrics.total)} icon={<UserRoundPlus/>}/><Kpi label="Engaged" value={metricValue(metrics.engaged)} icon={<MessagesSquare/>}/><Kpi label="Signed" value={metricValue(metrics.signed)} icon={<BadgeCheck/>}/><Kpi label="Scheduled" value={metricValue(metrics.scheduled)} icon={<Truck/>}/><Kpi label="Conversion" value={crmDataError?"—":`${metrics.conversion}%`} icon={<TrendingUp/>}/></section>
      ...
      {view==="Call Center"&&<CallCenter leads={leads} live={live} selected={selected} onSelect={setSelected} conversations={conversations} eventsLive={eventsLive} refresh={()=>setRevision(r=>r+1)}/>}
      {view==="Analytics"&&<AnalyticsView events={events}/>} 
    </main><SupportGuide navigate={next=>{if(next==="Call Center")setView("Call Center");else{setView("Operations");setTab(next as Tab);}}}/>
  </div>;
}

function Kpi({label,value,icon}:{label:string,value:any,icon:any}){return <div className="kpi"><span>{label}</span><div className="kpi-value"><div className="kpi-icon">{icon}</div><strong>{value}</strong></div></div>}
...
function AnalyticsView({events}:{events:BuddyEvent[]}){const counts=events.reduce((m:any,e)=>{m[e.type]=(m[e.type]||0)+1;return m;},{});const recent=events.slice(0,50);return <div className="analytics-view"><div className="table-card analytics-summary"><div className="table-title"><h2>Workflow Analytics</h2></div><div className="analytics-kpis"><Kpi label="Calls" value={events.filter(e=>e.type==="call.created").length} icon={<Phone/>}/><Kpi label="Customer Turns" value={counts["stt.transcript.final"]||0} icon={<MessageSquare/>}/><Kpi label="DocuSign Events" value={events.filter(e=>e.type.includes("docusign")).length} icon={<FileSignature/>}/><Kpi label="Delivery Events" value={events.filter(e=>e.type.includes("delivery")).length} icon={<CalendarDays/>}/></div></div><div className="table-card analytics-activity"><div className="table-title"><div><h2>Recent Activity</h2><p>Newest call, document and delivery events.</p></div></div>{recent.map(e=><div key={e.id} style={{display:"grid",gridTemplateColumns:"155px 190px 160px 1fr",gap:12,padding:"10px 4px",borderBottom:"1px solid #eef2f7",fontSize:12}}><span>{new Date(e.createdAt).toLocaleString()}</span><b>{e.type}</b><span>{e.contactId||"—"}</span><span>{e.text||e.payload?.productName||e.payload?.deliveryAt||e.payload?.status||"Workflow event"}</span></div>)}</div></div>}
