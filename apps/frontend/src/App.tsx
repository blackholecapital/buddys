...
export default function App(){
  ...
  const [revision,setRevision]=useState(0),[eventsLive,setEventsLive]=useState(false);
  useEffect(()=>{let alive=true;const load=()=>fetch("/api/contacts").then(r=>r.ok?r.json():Promise.reject()).then(payload=>{if(payload.ok===false)throw new Error();const rows=Array.isArray(payload)?payload:payload?.contacts||payload?.rows||payload?.data;if(!Array.isArray(rows))throw new Error();if(alive){const mapped=rows.map(normalizeContact);setLeads(mapped);setSelected(current=>mapped.find((l:Lead)=>l.id===current?.id)||mapped[0]||null);setLive(true);}}).catch(()=>{if(alive)setLive(false);});void load();const timer=window.setInterval(load,5000);return()=>{alive=false;clearInterval(timer);};},[revision]);
  ...
  return <div className="buddy-app">
    ...
    </nav><div className="side-status"><span className={live?"dot live":"dot"}/>{live?"Live data":"Data unavailable"}<small>AI sales workflow</small></div></aside>
    ...
    <main className="workspace"><header className="topbar crm-banner"><div className="crm-buddy" role="img" aria-label="Buddy standing in his showroom"/><div className="crm-brand-copy"><img src="/buddys/images/BHF_Logo.webp" alt="Buddy's Home Furnishings"/><div className="crm-title-copy"><h1>Personal Shopper Operations</h1><p>Your home. Your style. Your Buddy.</p></div></div><div className="crm-banner-actions"><a href="/buddys/#contact-form">Open sales experience <ExternalLink size={14}/></a><span className="crm-powered">Powered by <b>BLACK HOLE AI</b> · EILA OS</span></div></header>
      <section className="kpis"><Kpi label="Total Leads" value={metrics.total} icon={<UserRoundPlus/>}/><Kpi label="Engaged" value={metrics.engaged} icon={<MessagesSquare/>}/><Kpi label="Signed" value={metrics.signed} icon={<BadgeCheck/>}/><Kpi label="Scheduled" value={metrics.scheduled} icon={<Truck/>}/><Kpi label="Conversion" value={`${metrics.conversion}%`} icon={<TrendingUp/>}/></section>
