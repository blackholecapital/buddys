...
failVideo=true;
await click('buddyConnectButton');
assert.equal(requests.filter(r=>r.url==='/api/video/session').at(-1).body.experience,'showroom');
assert.equal(elements.buddyVideoModal.dataset.experience,'showroom');
assert.match(elements.buddyVideoMount.innerHTML,/buddy-reference-scene/,'Failure keeps showroom preview');
failVideo=false;
await click('buddyConnectButton');
assert.equal(elements.buddyVideoModal.dataset.experience,'showroom','Live connection retains showroom mode');
assert.match(elements.buddyVideoMount.innerHTML,/buddy-reference-scene/,'Waiting for live track retains scene');
