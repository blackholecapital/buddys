...
try{for(const width of [1672,1024,390]){const p=await browser.newPage({viewport:{width,height:941}});const errors=[];p.on('pageerror',e=>errors.push(e.message));await p.goto(`http://127.0.0.1:${server.address().port}/bb-demo/`);assert.equal(await p.locator('.look').count(),6);await p.locator('.try-look').nth(1).click();assert.equal(await p.locator('#modelLabel').textContent(),'Sequin Blazer Ensemble');await p.getByRole('button',{name:'Filter Wedding looks'}).click();assert.equal(await p.locator('.look').count(),3);await p.locator('.heart').first().click();assert.equal(await p.locator('.heart').first().getAttribute('aria-pressed'),'true');assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1),false);assert.equal(await p.locator('#chatInput').isDisabled(),true);assert.deepEqual(errors,[]);await p.getByRole('button',{name:'Filter Wedding looks'}).click();if(process.env.BB_SHOTS)await p.screenshot({path:`${process.env.BB_SHOTS}/bb-${width}.png`,fullPage:true});await p.close();}console.log('PASS: BB demo swaps, filters, hearts and responsive layout; live AI explicitly pending');}finally{await browser.close();server.close();}
