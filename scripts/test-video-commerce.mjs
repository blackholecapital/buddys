...
const env = {
  ...
  EMAIL:{ async fetch(req) { sent.push(await req.json()); return Response.json({ok:true}); } },
};
const adapterEnv = {
...
try {
  ...
  assert.equal(lead.ok,true); contactId=lead.contact.id;
  const session=await post('video/session',{contactId,interest:'wrong-client-override'});
  ...
  assert.equal((await post('video/session',{contactId})).workflow.phase,'awaiting-signature');
  ...
  // Simulate the provider callback; signature validation is a separate hardening gate.
  await concierge.fetch(new Request(`https://concierge.test/docusign/connect?contactId=${contactId}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({envelopeId:'envelope-test',status:'completed'})}),env,{});
  assert.equal((await post('video/action',{...auth,action:'contact-status'})).documentStatus,'Signed');
  assert.equal((await post('video/session',{contactId})).workflow.phase,'awaiting-delivery');
  ...
  const resumed=await post('video/session',{contactId});
  ...
  assert.equal((await post('video/session',{contactId:'missing'})).ok,false); assert.equal(mediaCalls,prior);
  const secret=env.INTERNAL_CALL_SECRET; env.INTERNAL_CALL_SECRET='';
  assert.equal((await post('video/session',{contactId})).ok,false); assert.equal(mediaCalls,prior); env.INTERNAL_CALL_SECRET=secret;
  failMedia=true; assert.equal((await post('video/session',{contactId})).ok,false); failMedia=false;
  malformedMedia=true; assert.equal((await post('video/session',{contactId})).ok,false); malformedMedia=false;
